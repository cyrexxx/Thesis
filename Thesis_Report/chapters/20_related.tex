...
The advantage of using \ac{GSSA} over other search methods is that the extrema is found in the least number of steps and every iteration requires only one additional data point. Thereby greatly reducing the complexity of the algorithm and hence the computation power and/or time required to lock-in onto the extrema. Golden ratio is usually represented with ${\phi}$ but less often with ${\tau}$. The flowchart in figure~\ref{fig:Golden_search} outlines the process to lock-in to a maxima, which forms an essential building block for the proposed algorithm later on (section~\ref{sec:proposed_algo_sec}).
